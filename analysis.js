...
async function analysis() {
  try {
    ...
    //sum of total billed
    const sumTotalBilled = trips.reduce((acc, obj) => {
      if (typeof obj.billedAmount === "string") {
        return acc + parseFloat(obj.billedAmount.replaceAll(",", ""));
      } else if (typeof obj.billedAmount === "number") {
        return acc + obj.billedAmount;
      } else {
        return acc;
      }
    }, 0);
    //total cash billed
    const sumTotalCashBilled = trips.reduce((acc, obj) => {
    ...
    // total non cash billed
    const sumNonTotalCashBilled = trips.reduce((acc, obj) => {
    ...
    //  new array [driver ids]
    const driverIds = trips.map((obj) => {
      return obj.driverID;
    });
 
    // filtering drivers ids
    ...
    let allDriverInfo = await Promise.allSettled(driverCont);
    ...
    // intializing driver's trips as an object
    let count = {};
   ...
   const maxDriverTrips = sortedEntries[0][1]
    ...
    const maxDriverBilled = trips.reduce((acc, obj) => {
    ...
    for (const key of driverUnique) {
      ...
      const totalAmountEarned = tripsPerDriver.reduce((acc, obj) => {
        if (typeof obj.billedAmount === "string") {
          return acc + parseFloat(obj.billedAmount.replaceAll(",", ""));
        } else if (typeof obj.billedAmount === "number") {
          return acc + obj.billedAmount;
    ...
    const result = {
      ...
      noOfDriversWithMoreThanOneVehicle: moreThanOneVehicle,
      mostTripsByDriver: {
